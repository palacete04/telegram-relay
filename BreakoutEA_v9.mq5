...
input int      MR_Lookback         = 12;
input double   MR_ATR_Mult         = 2.0;
input double   MR_TP_Pips          = 20.0;
input double   MR_SL_Pips          = 25.0;
   ...
   if(UsarRSI && !tradeRSIOpen && !cachedRSI)
   {
      ...
      if(barTime != lastRSIBar)
      {
         ...
         if(rsiHandle!=INVALID_HANDLE && CopyBuffer(rsiHandle,0,1,1,rsiBuffer)==1)
         {
            ...
            bool okBuy  = RSIUsarFiltroTendencia ? (rsi < RSISobrevendido && tendenciaAlcista) : (rsi < RSISobrevendido);
            bool okSell = RSIUsarFiltroTendencia ? (rsi > RSISobrecomprado && tendenciaBajista) : (rsi > RSISobrecomprado);
            if(okBuy)
            {
...
void SendHeartbeat()
{
   ...
   int res = WebRequest("POST", url, headers, 5000, post, result, responseHeaders);
   if(res != -1)
      Print("Heartbeat OK | Balance: ", DoubleToString(balance, 2));
}
